...
public abstract class NowPlayingBase : ComponentBase, IAsyncDisposable
{
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        try
        {
            ...
            // If a Plex album was restored from localStorage, broadcast album + track to peers
            // now so they have time to prepare before the user clicks Play. This mirrors the
            // timing of PlayPlexAlbumAsync which broadcasts at selection time, not at play time.
            if (_context.State is { IsJoined: true, PlexCurrentAlbumRatingKey: { } restoredKey }
                && _context.State.CurrentTrack?.Source == TrackSource.Plex)
            {
                await HandleAlbumSelectionBroadcast(restoredKey);
                await HandleTrackSelectionBroadcast(_context.State.CurrentTrack.Id);
            }
