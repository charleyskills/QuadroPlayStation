...
const engine = {
  ...
  // Resolves when the in-flight per-track clock sync completes. Set by
  // setSrc(), awaited and cleared by schedulePlay().
  ...
  _decode(url) {
    ...
    this._decodePromise = (async () => {
      try {
        // ── Cache API lookup ─────────────────────────────────────────────
        // Key by URL path only so rotating MinIO presigned query-strings
        // don't prevent hits — same file always has the same pathname.
        const cacheKey = this._cacheKey(url);
        const cache = await caches.open('syncaudio-v1').catch(() => null);
        const cachedResp = cache ? await cache.match(cacheKey).catch(() => null) : null;

        let merged;
        if (cachedResp) {
          const buf = await cachedResp.arrayBuffer();
          merged = new Uint8Array(buf);
          this._invoke('UpdateProgress', 0.9);
          this._reportHubProgress(0.9);
        } else {
          ...
          // Store in Cache API for next load (fire-and-forget; don't block decode).
          if (cache) {
            cache.put(cacheKey, new Response(merged.slice())).catch(() => {});
          }
        }

        // iOS Safari suspends AudioContext until a user-gesture calls ctx.resume().
        // decodeAudioData on a suspended context hangs indefinitely in WebKit.
        // OfflineAudioContext is exempt from the user-activation requirement
        // (it produces no real-time output), so it decodes reliably on iOS
        // regardless of whether the main context has been unlocked yet.
        // AudioBuffer is plain data and works with any AudioContext for playback.
        const OffCtx = window.OfflineAudioContext || window.webkitOfflineAudioContext;
        const decodeCtx = new OffCtx(2, 1, this.ctx.sampleRate || 44100);
        let _decodeTimeoutId;
        const buffer = await Promise.race([
          decodeCtx.decodeAudioData(merged.buffer),
          new Promise((_, reject) => {
            _decodeTimeoutId = setTimeout(
              () => reject(new Error('decodeAudioData timed out — codec not supported on this device')),
              20000);
          })
        ]);
        clearTimeout(_decodeTimeoutId);
        ...
        this._invoke('UpdateDecodeError', e?.message ?? String(e));
        throw e;
      } finally {
  ...
  setSrc(url) {
    ...
    // Re-calibrate clock in parallel with decode so schedulePlay() gets a
    // fresh offset for this track. Guard against not-yet-connected state.
    if (this.conn) this._clockSyncPromise = this.syncClock();
  ...
  // ─── Playback core ─────────────────────────────────────────────────
  async schedulePlay(playAtServerMs) {
    ...
    // Wait for the per-track clock sync started in setSrc() to finish so
    // this.offset is fresh before we compute the audio-clock fire time.
    if (this._clockSyncPromise) {
      await this._clockSyncPromise;
