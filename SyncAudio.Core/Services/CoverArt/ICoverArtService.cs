...
public interface ICoverArtService
{
    ...
    /// cover was found. <paramref name="size"/> is "thumb" (max 256 px lossy WebP), "large"
    /// (max 1500 px lossless WebP), or "full" (the original embedded picture, byte-for-byte).
    /// "large" falls back to "full" when the large variant has not yet been generated.
    /// Results are memory-cached with a sliding 30-minute expiry.
    /// </summary>
