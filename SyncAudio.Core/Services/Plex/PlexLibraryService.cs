    ...
    : IPlexLibraryService
{
        ...
        string query, int offset, int size, CancellationToken ct = default)
    {
        ...
        try
        {
            var (hits, total) = await server.SearchAlbumsByTitleAsync(
                ...
                offset, size, ct);
            return (hits.Select(h => MapAlbum(h, session)).ToList(), total);
        }
        catch (HttpRequestException ex) when (ex.StatusCode == HttpStatusCode.Unauthorized)
        {
    ...
    private static PlexAlbumSummary MapAlbum(PlexAlbumHit hit, PlexSession session) => new(
        ...
        CoverUrl: hit.ThumbKey is null ? null
            : $"/plex/cover/{Uri.EscapeDataString(session.MachineIdentifier)}/{Uri.EscapeDataString(hit.RatingKey)}?thumb={Uri.EscapeDataString(hit.ThumbKey)}");
